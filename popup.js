document.getElementById("btnEscanear").addEventListener("click", async () => {
  ...
  const options = {
    ...
    ordenar: document.getElementById("filtroAlfabetico").checked, // ¡Corregido el paréntesis!
  ...
  const [tab] = await browser.tabs.query({ active: true, currentWindow: true }); // ¡Corregida la W mayúscula!
  ...
  browser.tabs.sendMessage(tab.id, { action: "iniciarEscaneo", opciones: options }); // ¡Corregido tab.id y alineado con la constante options!
